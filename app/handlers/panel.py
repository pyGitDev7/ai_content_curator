...
async def cb_crawl_one(callback: CallbackQuery) -> None:
    ...
    async with async_session_factory() as s:
        ...
        await s.commit()
    await callback.message.answer(f"✅ «{src.name}»: {new} جدید از {len(raws)} یافت شده")
...
async def cb_recent(callback: CallbackQuery) -> None:
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton(text="◀️", callback_data=f"p:recent:{page - 1}"))
    if len(items) == pp:
        nav.append(InlineKeyboardButton(text="▶️", callback_data=f"p:recent:{page + 1}"))
    if nav:
        btns.append(nav)

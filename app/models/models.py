...
class Source(Base):
    ...
    type: Mapped[str] = mapped_column(String(50), nullable=False, index=True)  # rss, telegram, twitter, reddit, website, github, hackernews, arxiv
    ...
    content_items: Mapped[list["ContentItem"]] = relationship(
        "ContentItem", back_populates="source", lazy="selectin"
    )
...
class ContentItem(Base):
    ...
    tags_json: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON list of hashtags
    ...
    delivery_logs: Mapped[list["DeliveredLog"]] = relationship(
        "DeliveredLog", back_populates="content_item", lazy="selectin"
    )

    __table_args__ = (
        Index("ix_content_score_desc", "score"),
        Index("ix_content_created_desc", "created_at"),
    )
...
class DeliveredLog(Base):
    ...
    delivered_at: Mapped[_dt.datetime] = mapped_column(
        DateTime, server_default=func.now(), nullable=False
    )

    content_item: Mapped["ContentItem"] = relationship(
        "ContentItem", back_populates="delivery_logs"
    )
